package com.example.webexception.dao;
...
public class TicketDao {
    ...
    public TicketDao() {
        if (ticketList.size() == 0) {
            //初始化数据
            for (int i = 1; i <= 10; ++i) {
                Ticket ticket = Ticket.builder().creator("admin").company("中山领航科技公司")
                        .money(1000.0 * i).title("办公器材" + i).body("2020年新购置办公设备、电脑设备" + i).type("银行汇票")
